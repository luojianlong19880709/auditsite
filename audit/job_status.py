		...
		if 'beginTimeshow_user' in req.GET:
			...
			if req.GET.has_key('search_user'):
				...
				for i in range(24):
					time1 = st + ' ' + str(i)
					time2 = st + ' ' + str(i+1)
					cmd = 'select sum(mapsTotal),sum(reducesTotal) from audit_job_status_yz where finishTime>="%s" and finishTime<"%s" and jobuser="%s"' %(time1,time2,jobuser)
					data1 = mysqlconn.mysql_conn(cmd)
					if data1[0][0] or data1[0][1]:
					...
					cmd = 'select sum(sucesscount),sum(failcount) from audit_job_total_yz where finishTime>="%s" and finishTime<"%s" and jobuser="%s"' %(time1,time2,jobuser)
					data2 = mysqlconn.mysql_conn(cmd)
					if data2[0][0] or data2[0][1]:
