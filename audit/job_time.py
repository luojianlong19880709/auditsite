...
def job_time_status(req):
	...
	userlist = Job_status_yz.objects.values('jobuser').distinct()
        ...
        for i in range(24):
                ...
                cmd = 'select sum(computeTime) from audit_job_status_yz where finishTime>="%s" and finishTime<"%s"' %(time1,time2)
		...
		if 'beginTimeshow' in req.GET:
			...
			for i in range(24):
        		...
        		return render_to_response('job_time.html',{'userlist':userlist,'startTime':startTime,'timelist':timelist})
		...
		if 'beginTimeshow_user' in req.GET:
			...
			if req.GET.has_key('search_user'):
				...
				for i in range(24):
					time1 = st + ' ' + str(i)
					time2 = st + ' ' + str(i+1)
					cmd = 'select sum(computeTime) from audit_job_status_yz where finishTime>="%s" and finishTime<"%s" and jobuser="%s"' %(time1,time2,jobuser)
					data1 = mysqlconn.mysql_conn(cmd)
					if data1[0][0]:
				...
				return render_to_response('job_time.html',{'userlist':userlist,'jobuser':jobuser,'st':st,'timelist_user':timelist_user})
