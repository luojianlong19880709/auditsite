...
from audit.models import User1
...
def user_list(req):
	...
	if 'user_name' in req.GET:
		...
		datas = User1.objects.filter(user_name=user_name)

		return render_to_response('user_list1.html',{'datas':datas})
...
def user_del(req,user_name):
	userinfo = User1.objects.get(user_name=user_name)
	userinfo.delete()


	
	return render_to_response('user_list1.html',{})




def user_update(req,user_name):
	userinfo = User1.objects.filter(user_name=user_name)
	if 'user_name' in req.GET:
		applicant = req.GET.get('applicant')
		department = req.GET.get('department')
		leader = req.GET.get('leader')
	...
	return render_to_response('user_info1.html',{'userinfo':userinfo})


def user_info(req,user_name):
	userinfo = User1.objects.filter(user_name=user_name)
	
	return render_to_response('user_profile1.html',{'userinfo':userinfo})
