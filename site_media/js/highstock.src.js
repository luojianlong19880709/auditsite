...
Axis.prototype = {
	 ...
	 * we have finally established min and max.lxb
	 */
	adjustForMinRange: function () {
		...
		if (axis.isXAxis && axis.minRange === UNDEFINED && !axis.isLog) {
			...
			if (defined(options.min) || defined(options.max)) {
				...
				axis.minRange = mathMin(closestDataRange * 24, axis.dataMax - axis.dataMin);
			}
		}
	...
	setTickPositions: function (secondPass) {
		...
		if (axis.range) {
			...
			//axis.userMax = axis.min;// modify by lxb
			if (secondPass) {
...
MouseTracker.prototype = {
	...
	init: function (chart, options) {
		...
		if (options.enabled) {
			...
			if(chart.tooltip!=null || chart.tooltip!==undefined)
			chart.tooltip.tick();}, 32);
...
var DATA_GROUPING = 'dataGrouping',
	...
	commonOptions = {
			...
			dateTimeLabelFormats: hash( 
				...
				MINUTE, ['%A, %b%e日 %H:%M', '%A, %b %e, %H:%M', '-%H:%M'],
				HOUR, ['%A, %b%e日%H点', '%A, %b %e, %H:%M', '-%H:%M'],
				DAY, ['%A, %b%e日, %Y', '%A, %b %e', '-%A, %b %e, %Y'],
